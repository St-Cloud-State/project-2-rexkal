...
public class UserInterface {
    ...
    public void placeOrder() {
        String clientID = getToken("Enter client ID");
        String result = warehouse.processClientOrder(clientID);
        System.out.println(result);
    }
    ...
    public int getCommand() {
        do {
            try {
                String value = getToken("Enter command (type " + HELP + " for help): ");
                int command = Integer.parseInt(value);
                if (command >= EXIT && command <= HELP) {
                    return command;
                } else {
                    System.out.println("Invalid command. Please enter a number between " + EXIT + " and " + HELP + ".");
                }
            } catch (NumberFormatException nfe) {
